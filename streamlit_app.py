...
st.set_page_config(page_title="GYF-Care", layout="wide", initial_sidebar_state="expanded")
...
st.markdown('<div class="main-title">GYF-Care - Gestión de Asignación de Pacientes</div>', unsafe_allow_html=True)
st.markdown('<div class="subtitle">Sistema inteligente de asignación con UFDS y rutas reales</div>', unsafe_allow_html=True)
...
if "graph_builder" not in st.session_state:
    with st.spinner("Inicializando estructuras de grafos..."):
        ...
        hospitales_df_original = cargar_datos("data/pacientesprueba.csv", "data/hospitalesprueba.csv")[1]
...
if "assignment_service" not in st.session_state:
    with st.spinner("Inicializando servicio de asignación..."):
        hospitales_df_original = cargar_datos("data/pacientesprueba.csv", "data/hospitalesprueba.csv")[1]
        st.session_state["assignment_service"] = AssignmentService(hospitales_df_original)
...
# ------------------------
with st.sidebar:
    st.image("https://w7.pngwing.com/pngs/870/305/png-transparent-peru-flag-thumbnail.png", width=80)
    ...
    with col2:
        ...
        departamentos = hosp_state["Departamento"].nunique() if "Departamento" in hosp_state.columns else 0
    ...
    menu = st.radio(
        ...
        ["Mapa Interactivo", "Visualización de Grafos", "Asignaciones"],
